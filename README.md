...
```sql
SELECT "ВЕЩИ"."НАЗВАНИЕ", "ВЕЩИ"."ПРОКАТНАЯ ЦЕНА ЗА НЕДЕЛЮ, РУБ" 
FROM "ВЕЩИ", "ПРОКАТ", "ЗАКАЗЧИК", "ПУНКТ ПРОКАТА" 
WHERE "ПРОКАТ"."ВЕЩЬ" = "ВЕЩИ"."ИДЕНТИФИКАТОР" 
AND "ПРОКАТ"."КЛИЕНТ" = "ЗАКАЗЧИК"."ИДЕНТИФИКАТОР"
AND "ЗАКАЗЧИК"."ФАМИЛИЯ" = 'Кожаков' 
AND "ПРОКАТ"."ПУНКТ ПРОКАТА" = "ПУНКТ ПРОКАТА"."ИДЕНТИФИКАТОР" 
AND "ЗАКАЗЧИК"."РАЙОН" <> "ПУНКТ ПРОКАТА"."РАСПОЛОЖЕНИЕ"
```
| НАЗВАНИЕ | ПРОКАТНАЯ ЦЕНА ЗА НЕДЕЛЮ, РУБ |
| :--- | :--- |
| Весы детские | 1500 |
| Холодильник | 12000 |
| Телевизор | 10000 |
...
```sql
SELECT "ВЕЩИ"."НАЗВАНИЕ", "ПРОКАТ"."СРОК ПРОКАТА"
FROM "ПРОКАТ", "ВЕЩИ"
WHERE "ПРОКАТ"."ВЕЩЬ" = "ВЕЩИ"."ИДЕНТИФИКАТОР"
AND "СРОК ПРОКАТА" >= ALL (SELECT "СРОК ПРОКАТА" FROM "ПРОКАТ" WHERE "ПРОКАТ"."ДАТА" IN ('Август', 'Июнь', 'Июль'))
...
```sql
SELECT "ВЕЩИ"."ПРОКАТНАЯ ЦЕНА ЗА НЕДЕЛЮ, РУБ", "ПУНКТ ПРОКАТА"."НОМЕР"
FROM "ПРОКАТ", "ПУНКТ ПРОКАТА", "ВЕЩИ"
WHERE "ПРОКАТ"."ПУНКТ ПРОКАТА" = "ПУНКТ ПРОКАТА"."ИДЕНТИФИКАТОР"
AND "ПРОКАТ"."ВЕЩЬ" = "ВЕЩИ"."ИДЕНТИФИКАТОР"
...
```sql
SELECT DISTINCT "ЗАКАЗЧИК"."ФАМИЛИЯ"
FROM "ПРОКАТ", "ВЕЩИ", "ЗАКАЗЧИК"
WHERE "СКИДКА, %" = ANY(
    SELECT "СКИДКА, %"
...
```sql
SELECT "РАЙОН" FROM "ЗАКАЗЧИК"
UNION
SELECT "РАСПОЛОЖЕНИЕ" FROM "ПУНКТ ПРОКАТА"
```
...
```sql
SELECT "НАЗВАНИЕ" FROM "ВЕЩИ", "ПРОКАТ"
WHERE EXISTS(
    SELECT "НОМЕР"
...
FROM "ПРОКАТ", "ПУНКТ ПРОКАТА", "ЗАКАЗЧИК"
WHERE "ПРОКАТ"."ПУНКТ ПРОКАТА" = "ПУНКТ ПРОКАТА"."ИДЕНТИФИКАТОР"
AND "ПУНКТ ПРОКАТА"."НОМЕР" = 'N8'
GROUP BY "ЗАКАЗЧИК"."ФАМИЛИЯ" LIMIT 1
```
...
```sql
SELECT "ВЕЩИ"."НАЗВАНИЕ", sum("СУММА, РУБ") FROM "ВЕЩИ", "ПРОКАТ" 
WHERE "ПРОКАТ"."ВЕЩЬ" = "ВЕЩИ"."ИДЕНТИФИКАТОР" 
GROUP BY "ВЕЩИ"."НАЗВАНИЕ"
```

| НАЗВАНИЕ | sum |
| :--- | :--- |
| Холодильник | 48000 |
| Часы напольные | 50000 |
| Телевизор | 180000 |
| Весы детские | 6000 |
| Утюг | 24000 |
| Радиоприемник | 140000 |
| Часы настенные | 3000 |
